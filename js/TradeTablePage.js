function startTablePage(){
      removeDOM(false, 'start')
      renderPageName('Таблица товаров')
      renderTablePrice('content')
}

function renderTablePrice(e){                                                   //создать таблицу товаров
      let mapName = new Map()
      ...
      for(key of mapName.keys()){
            ...
            let iterat = ''
            for(a in arr){
                  iterat += `<td data-locat='${key.name}' data-item='${arr[a][2]}' data-trade='buy' data-vertik='${vert}' data-goriz='${swit}' class='vertik${vert} goriz${swit}' onclick='changeValue(this)' style='cursor: pointer'>${arr[a][0]}</td>`
                  iterat += `<td data-locat='${key.name}' data-item='${arr[a][2]}' data-trade='sell' data-vertik='${vert}' data-goriz='${swit}' class='vertik${vert} goriz${swit}' onclick='changeValue(this)' style='cursor: pointer'>${arr[a][1]}</td>`
                  swit++
            }
...
function changeValue(n){                                                            //вызывает окно для изменения значения
      ...
      emptyModal.onclick = function(){return removeDOM(true,'emptyModal','modalChangePrice')}
      ...
      modalChangePrice.innerHTML = `<p style="text-align: center;">Ввести значения</p> <input id="changeInput">${n.innerText}</input> <button data-locat='${locat}' data-item='${item}' data-trade='${trade}' onclick='funcChangeValue(this)'>Ввести</button>`
      ...
      if (locat[trade]){
            ...
            for(let i in locat[trade]){
                  if(locat[trade][i][0] == item){
                        ...
                        removeDOM(false, 'content')
                        renderTablePrice('content')
                        setTableServer(universe, 'http://127.0.0.1:3002')
                        removeDOM(true,'emptyModal','modalChangePrice')
